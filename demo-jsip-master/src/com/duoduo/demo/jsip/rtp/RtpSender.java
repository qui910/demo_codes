...
public class RtpSender {
	...
	public static void main(String[] args) {
		try {// 设置本地地址和远端地址都为本机地址，自己传给自己，试验用
			...
			new RtpSender(local, target);
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
	}

	public RtpSender(SessionAddress local, SessionAddress target) {
		CaptureDeviceInfo info = (CaptureDeviceInfo) CaptureDeviceManager.getDeviceList(new VideoFormat(null)).get(0);
		// CaptureDeviceInfo info = (CaptureDeviceInfo) CaptureDeviceManager.getDeviceList(new
	...
	private static boolean setVideoTrackFormat(TrackControl[] tracks) {
		if (tracks == null || tracks.length < 1) return false;
