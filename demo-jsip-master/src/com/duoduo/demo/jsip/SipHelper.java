...
class SipHelper {
	...
	private List<ViaHeader> createViaHeaders() throws ParseException, SipException, InvalidArgumentException {
	...
	private ContactHeader createContactHeader(SipProfile profile, String ip, int port) throws ParseException,
			SipException {
		SipURI contactURI = (ip == null) ? createSipUri(profile.getUserName(), profile.getProtocol(),
				getListeningPoint()) : createSipUri(profile.getUserName(), profile.getProtocol(), ip, port);
	...
	public ClientTransaction sendOptions(SipProfile caller, SipProfile callee, String tag) throws SipException {
		try {
			Request request = (caller == callee) ? createRequest(Request.OPTIONS, caller, tag) : createRequest(
					Request.OPTIONS, caller, callee, tag);
	...
	private Request createRequest(String requestType, SipProfile userProfile, String tag) throws ParseException,
			SipException, InvalidArgumentException {
		FromHeader fromHeader = createFromHeader(userProfile, tag);
		ToHeader toHeader = createToHeader(userProfile);

		String replaceStr = Pattern.quote(userProfile.getUserName() + "@");
		SipURI requestURI = mAddressFactory.createSipURI(userProfile.getUserName(), userProfile.getUriString()
		...
		Request request = mMessageFactory.createRequest(requestURI, requestType, callIdHeader, cSeqHeader, fromHeader,
	...
	public ClientTransaction handleChallenge(ResponseEvent responseEvent, AccountManager accountManager)
			throws SipException {
		AuthenticationHelper authenticationHelper = ((SipStackExt) mSipStack).getAuthenticationHelper(accountManager,
				mHeaderFactory);
		ClientTransaction tid = responseEvent.getClientTransaction();
		ClientTransaction ct = authenticationHelper.handleChallenge(responseEvent.getResponse(), tid, mSipProvider, 5);
		if (DEBUG) System.out.println("send request with challenge response: " + ct.getRequest());
		ct.sendRequest();
		return ct;
	}

	private Request createRequest(String requestType, SipProfile caller, SipProfile callee, String tag)
	...
	public ClientTransaction sendInvite(SipProfile caller, SipProfile callee, String sessionDescription, String tag,
			ReferredByHeader referredBy, String replaces) throws SipException {
		try {
			...
			ClientTransaction clientTransaction = mSipProvider.getNewClientTransaction(request);
			if (DEBUG) System.out.println("send INVITE: " + request);
			clientTransaction.sendRequest();
			return clientTransaction;
		} catch (Exception e) {
	...
	public ClientTransaction sendReinvite(Dialog dialog, String sessionDescription) throws SipException {
		try {
			...
			ClientTransaction clientTransaction = mSipProvider.getNewClientTransaction(request);
			if (DEBUG) System.out.println("send RE-INVITE: " + request);
	...
	public ServerTransaction sendRinging(RequestEvent event, String tag) throws SipException {
		try {
			...
			if (DEBUG) System.out.println("send RINGING: " + response);
	 ...
	 * @param event the INVITE request event
	 */
	public ServerTransaction sendInviteOk(RequestEvent event, SipProfile localProfile, String sessionDescription,
			ServerTransaction inviteTransaction, String externalIp, int externalPort) throws SipException {
		try {
			Request request = event.getRequest();
			...
			if (inviteTransaction.getState() != TransactionState.COMPLETED) {
				if (DEBUG) System.out.println("send OK: " + response);
	...
	public void sendInviteBusyHere(RequestEvent event, ServerTransaction inviteTransaction) throws SipException {
		try {
			...
			if (inviteTransaction.getState() != TransactionState.COMPLETED) {
				if (DEBUG) System.out.println("send BUSY HERE: " + response);
				inviteTransaction.sendResponse(response);
			}
		} catch (Exception e) {
	 ...
	 */
	public void sendInviteAck(ResponseEvent event, Dialog dialog) throws SipException, InvalidArgumentException {
		Response response = event.getResponse();
		long cseq = ((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getSeqNumber();
		Request ack = dialog.createAck(cseq);
		if (DEBUG) System.out.println("send ACK: " + ack);
		dialog.sendAck(ack);
	}

	public void sendBye(Dialog dialog) throws SipException {
		Request byeRequest = dialog.createRequest(Request.BYE);
		if (DEBUG) System.out.println("send BYE: " + byeRequest);
	...
	public void sendCancel(ClientTransaction inviteTransaction) throws SipException {
		...
		if (DEBUG) System.out.println("send CANCEL: " + cancelRequest);
		mSipProvider.getNewClientTransaction(cancelRequest).sendRequest();
	}

	public void sendResponse(RequestEvent event, int responseCode) throws SipException, InvalidArgumentException {
		try {
			Request request = event.getRequest();
	...
	public void sendReferNotify(Dialog dialog, String content) throws SipException {
		try {
			...
			if (DEBUG) System.out.println("send NOTIFY: " + request);
	...
	public void sendInviteRequestTerminated(Request inviteRequest, ServerTransaction inviteTransaction)
			throws SipException {
		try {
			Response response = mMessageFactory.createResponse(Response.REQUEST_TERMINATED, inviteRequest);
			if (DEBUG) System.out.println("send response: " + response);
			inviteTransaction.sendResponse(response);
		} catch (ParseException e) {
